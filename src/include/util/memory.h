 ...
 * @return A pointer to the allocated chunk of memory. If allocation fails, returns a NULL pointer.
 *         It is the caller's responsibility to decide how to handle the error.
 */
[[nodiscard]] inline void *MallocAligned(const std::size_t size, const std::size_t alignment) {
  TPL_ASSERT(MathUtil::IsPowerOf2(alignment), "Alignment must be a power of two");
  TPL_ASSERT(MathUtil::IsAligned(alignment, sizeof(void *)),
 ...
 * Physical memory is demand-paged in upon first access. This can potentially be a performance-issue
 * when accessing newly allocated pages in a hot loop. For this reason, the caller can request that
 * allocated pages be pre-faulted and populated immediately by setting @em populate to true.
 *
 ...
 */
[[nodiscard]] inline void *MallocHuge(const std::size_t size, const bool populate) {
 ...
 */
template <typename T>
[[nodiscard]] inline T *MallocHugeArray(const std::size_t num_elems, const bool populate) {
 ...
 */
inline void FreeHuge(void *ptr, const std::size_t size) {
  TPL_ASSERT(ptr != nullptr, "Cannot free NULL pointer");
  TPL_ASSERT(size != 0, "Cannot free zero-sized memory");
  munmap(ptr, size);
}
 ...
 */
template <typename T>
inline void FreeHugeArray(T arr[], const std::size_t num_elems) {
  TPL_ASSERT(arr != nullptr, "Cannot free NULL pointer");
  TPL_ASSERT(num_elems != 0, "Cannot free zero-sized memory");
 ...
 * prefetch is intended for a subsequent read or write, and the degree of locality for the address.
 * @tparam READ A boolean indicating whether the prefetch request is for memory that will be
 *              read-only or written to.
 * @tparam LOCALITY A hint indicating the temporal locality (i.e., reuse) of the memory address. For
 *                  high-reuse memory, the fetch will attempt to place the memory into L1 cache. For
 *                  low-reuse memory, the fetch will attempt to place the memory into
 *                  slower-but-larger L3 cache.
 * @param addr The address to issue a prefetch for.
 */
template <bool READ, Locality LOCALITY>
inline void Prefetch(const void *const addr) {
