...
inline Vec8 &Vec8::Load(const i64 *ptr) {
...
class Vec16 : public Vec512b {
 public:
  ...
  /// pointer array.
  Vec16 &Load(const i8 *ptr);
  Vec16 &Load(const u8 *ptr) { return Load(reinterpret_cast<const i8 *>(ptr)); }
...
template <>
inline Vec16 &Vec16::Gather<i32>(const i32 *ptr, const Vec16 &pos) {
  reg_ = _mm512_i32gather_epi32(pos, ptr, 4);
  return *this;
}

inline void Vec16::Store(i8 *arr) const {
  const __mmask16 all(~(u16)0);
  _mm512_mask_cvtepi32_storeu_epi8(reinterpret_cast<void *>(arr), all, reg());
}

inline void Vec16::Store(i16 *arr) const {
  const __mmask16 all(~(u16)0);
...
inline Vec16 operator>>(const Vec16 &a, const u32 shift) {
  return Vec16(_mm512_srli_epi32(a, shift));
}

inline Vec16 &operator>>=(Vec16 &a, const u32 shift) {
  a = a >> shift;
  return a;
}
