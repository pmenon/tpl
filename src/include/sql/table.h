...
class Table {
 public:
  ...
  /**
   * Create a new table with the provided unique OID, name and schema.
   * @param id The desired ID of the table.
   * @param name The name of the table.
   * @param schema The physical schema of the table.
   */
  Table(uint16_t id, std::string name, std::unique_ptr<Schema> schema)
      : id_(id), name_(std::move(name)), schema_(std::move(schema)), num_tuples_(0) {}

  /**
   * Take ownership of and append the given block of table data to the end of the table.
  ...
  /**
   * @return A const-iterator positioned at the first block in the table's block list.
   */
  Table::BlockList::const_iterator begin() const { return blocks_.begin(); }

  /**
   * @return An iterator positioned at the first block in the table's block list.
   */
  Table::BlockList::iterator begin() { return blocks_.begin(); }

  /**
   * @return A const-iterator positioned at one past the end of the table's block list.
   */
  Table::BlockList::const_iterator end() const { return blocks_.end(); }

  /**
   * @return An iterator positioned at one past the end of the table's block list.
   */
...
class TableBlockIterator {
 public:
  ...
  /**
   * Create an iterator over a subset of the blocks in the table with the provided OID. Iteration
  ...
  /**
   * @return The current block the iterator is positioned at.
   */
