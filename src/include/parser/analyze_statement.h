...
class AnalyzeStatement : public SQLStatement {
 public:
   ...
   */
  AnalyzeStatement(std::shared_ptr<TableRef> analyze_table,
                   std::shared_ptr<std::vector<std::string>> analyze_columns)
   ...
   */
  std::shared_ptr<std::vector<std::string>> GetAnalyzeColumns() {
    return analyze_columns_;
  }
