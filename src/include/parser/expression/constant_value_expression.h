...
class ConstantValueExpression : public AbstractExpression {
 public:
  ...
  explicit ConstantValueExpression(const type::TransientValue &value)
      : AbstractExpression(ExpressionType::VALUE_CONSTANT, value.Type(), {}),
        value_(value) {}

  common::hash_t Hash() const override {
    return common::HashUtil::CombineHashes(AbstractExpression::Hash(),
                                           value_.Hash());
  }
  ...
  bool operator==(const AbstractExpression &other) const override {
    ...
    auto const &const_expr =
        reinterpret_cast<const ConstantValueExpression &>(other);
    return value_ == const_expr.GetValue();
  }

  std::unique_ptr<AbstractExpression> Copy() const override {
    return std::make_unique<ConstantValueExpression>(*this);
  }
