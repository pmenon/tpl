 ...
 * traverse the version chain and find the relevant tuple version: pointer to
 * the next record, timestamp of the transaction that created this record,
 * pointer to the data table, and the tuple slot.
 */
class UndoRecord {
 public:
   ...
   * @return Timestamp up to which the old projected row was visible.
   */
  const std::atomic<transaction::timestamp_t> &Timestamp() const {
    return timestamp_;
  }
   ...
   * @warning this can be nullptr if it was never installed in a version chain,
   * or unlinked by GC
   */
  ...
  uint32_t Size() const {
    return static_cast<uint32_t>(
        sizeof(UndoRecord) +
        (type_ == DeltaRecordType::UPDATE ? Delta()->Size() : 0));
  }

  /**
   * @param redo the redo changes to be applied
   * @return size of the UndoRecord which can store the delta resulting from
   * applying redo in memory, in bytes
   */
  ...
  /**
   * Calculates the size of this UndoRecord, including all members, values, and
   * bitmap
   *
  ...
  static uint32_t Size(const ProjectedRowInitializer &initializer) {
    return static_cast<uint32_t>(sizeof(UndoRecord)) +
   ...
   * @param timestamp timestamp of the transaction that generated this
   * UndoRecord
   ...
   * @return pointer to the initialized UndoRecord
   */
  static UndoRecord *InitializeUpdate(
      byte *const head, const transaction::timestamp_t timestamp,
      const TupleSlot slot, DataTable *const table,
  ...
  /**
   * Populates the UndoRecord's members to hold an update based on the redo
   * changes that this UndoRecord is supposed to log.
   *
   * @param head pointer to the byte buffer to initialize as a UndoRecord
   * @param timestamp timestamp of the transaction that generated this
   * UndoRecord
   ...
   * @return pointer to the initialized UndoRecord
   */
  static UndoRecord *InitializeUpdate(byte *const head,
                                      const transaction::timestamp_t timestamp,
                                      const TupleSlot slot,
                                      DataTable *const table,
 ...
 private:
  ...
  DataTable *table_;
  TupleSlot slot_;
  // This needs to be aligned to 8 bytes to ensure the real size of UndoRecord
