...
// This is the maximum number of 8-byte slots to pack into a single
// CompactIntsKey template. This constraint is not arbitrary and cannot be
// increased beyond 256 bits until AVX-512 is more widely available.
...
class CompactIntsKey {
 public:
  ...
  static_assert(
      KeySize > 0 &&
      KeySize <= INTSKEY_MAX_SLOTS);  // size must be no greater than 256-bits
  static_assert(key_size_byte % sizeof(uintptr_t) ==
                0);  // size must be multiple of 8 bytes
  ...
  /**
   * Set the CompactIntsKey's data based on a ProjectedRow and associated index
   * metadata
   * @param from ProjectedRow to generate CompactIntsKey representation of
   * @param metadata index information, primarily attribute sizes and the
   * precomputed offsets to translate PR layout to CompactIntsKey
   */
  void SetFromProjectedRow(const storage::ProjectedRow &from,
    ...
    TERRIER_ASSERT(attr_sizes.size() == from.NumColumns(),
                   "attr_sizes and ProjectedRow must be equal in size.");
    ...
    for (uint8_t i = 0; i < from.NumColumns(); i++) {
      TERRIER_ASSERT(compact_ints_offsets[i] + attr_sizes[i] <= key_size_byte,
                     "out of bounds");
      CopyAttrFromProjection(from, static_cast<uint16_t>(from.ColumnIds()[i]),
                             attr_sizes[i], compact_ints_offsets[i]);
    }
  }
 ...
 private:
  ...
  void CopyAttrFromProjection(const storage::ProjectedRow &from,
                              const uint16_t projection_list_offset,
                              const uint8_t attr_size,
                              const uint8_t compact_ints_offset) {
    const byte *const stored_attr =
        from.AccessWithNullCheck(projection_list_offset);
    TERRIER_ASSERT(stored_attr != nullptr,
                   "Cannot index a nullable attribute with CompactIntsKey.");
    switch (attr_size) {
      case sizeof(int8_t): {
   ...
   *      XCHG AH, AL
   */
  static uint16_t TwoBytesToBigEndian(const uint16_t data) {
    return htobe16(data);
  }
   ...
   *      BSWAP EAX
   */
  static uint32_t FourBytesToBigEndian(const uint32_t data) {
    return htobe32(data);
  }
   ...
   */
  static uint64_t EightBytesToBigEndian(const uint64_t data) {
    return htobe64(data);
  }
   ...
   */
  static uint16_t TwoBytesToHostEndian(const uint16_t data) {
    return be16toh(data);
  }
   ...
   * order
   */
  static uint32_t FourBytesToHostEndian(const uint32_t data) {
    return be32toh(data);
  }
   ...
   * order
   */
  static uint64_t EightBytesToHostEndian(const uint64_t data) {
    return be64toh(data);
  }
  ...
  static uint8_t ToBigEndian(const int8_t data) {
    return static_cast<uint8_t>(data);
  }

  static uint16_t ToBigEndian(const uint16_t data) {
    return TwoBytesToBigEndian(data);
  }

  static uint16_t ToBigEndian(const int16_t data) {
    return TwoBytesToBigEndian(static_cast<uint16_t>(data));
  }

  static uint32_t ToBigEndian(const uint32_t data) {
    return FourBytesToBigEndian(data);
  }

  static uint32_t ToBigEndian(const int32_t data) {
    return FourBytesToBigEndian(static_cast<uint32_t>(data));
  }

  static uint64_t ToBigEndian(const uint64_t data) {
    return EightBytesToBigEndian(data);
  }

  static uint64_t ToBigEndian(const int64_t data) {
  ...
  static uint8_t ToHostEndian(const int8_t data) {
    return static_cast<uint8_t>(data);
  }

  static uint16_t ToHostEndian(const uint16_t data) {
    return TwoBytesToHostEndian(data);
  }

  static uint16_t ToHostEndian(const int16_t data) {
    return TwoBytesToHostEndian(static_cast<uint16_t>(data));
  }

  static uint32_t ToHostEndian(const uint32_t data) {
    return FourBytesToHostEndian(data);
  }

  static uint32_t ToHostEndian(const int32_t data) {
    return FourBytesToHostEndian(static_cast<uint32_t>(data));
  }

  static uint64_t ToHostEndian(const uint64_t data) {
    return EightBytesToHostEndian(data);
  }

  static uint64_t ToHostEndian(const int64_t data) {
  ...
  static IntType SignFlip(IntType data) {
    ...
    auto mask = static_cast<IntType>(static_cast<IntType>(0x1)
                                     << (sizeof(IntType) * 8UL - 1));
    return data ^ mask;
  }

  /**
   * Adds an integer into key_data_ in compact form
   * @tparam IntType must be of the following 4 types: int8_t; int16_t; int32_t;
   * int64_t
   ...
   * @tparam IntType must be of the following 4 types: uint8_t; uint16_t;
   * uint32_t; uint64_t
   * @param data value to be inserted
   * @param offset byte offset within key_data_ to write to
   */
  ...
  void AddUnsignedInteger(const IntType data, const size_t offset) {
    ...
    TERRIER_ASSERT(offset + sizeof(IntType) <= key_size_byte,
                   "Out of bounds access on key_data_.");
...
/**
 * Implements std::hash for CompactIntsKey. Allows the class to be used with STL
...
struct hash<terrier::storage::index::CompactIntsKey<KeySize>> {
   ...
   */
  size_t operator()(
      const terrier::storage::index::CompactIntsKey<KeySize> &key) const {
    const auto *const ptr = key.KeyData();
    return terrier::common::HashUtil::HashBytes(
        ptr, terrier::storage::index::CompactIntsKey<KeySize>::key_size_byte);
  }
};

/**
 * Implements std::equal_to for CompactIntsKey. Allows the class to be used with
 * STL containers and the BwTree index.
 * @tparam KeySize number of 8-byte fields to use. Valid range is 1 through 4.
 */
template <uint8_t KeySize>
struct equal_to<terrier::storage::index::CompactIntsKey<KeySize>> {
  /**
   * Due to the KeySize constraints this should be optimized to a single SIMD
   * instruction.
   ...
   */
  bool operator()(
      const terrier::storage::index::CompactIntsKey<KeySize> &lhs,
      const terrier::storage::index::CompactIntsKey<KeySize> &rhs) const {
    return std::memcmp(lhs.KeyData(), rhs.KeyData(),
                       terrier::storage::index::CompactIntsKey<
                           KeySize>::key_size_byte) == 0;
  }
};

/**
 * Implements std::less for CompactIntsKey. Allows the class to be used with STL
 * containers and the BwTree index.
 * @tparam KeySize number of 8-byte fields to use. Valid range is 1 through 4.
 */
template <uint8_t KeySize>
