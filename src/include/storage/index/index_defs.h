...
/**
 * A column of the index key has an identifier, type, and describes whether it
 * can be null.
 */
class IndexKeyColumn {
 public:
   ...
   */
  IndexKeyColumn(catalog::indexkeycol_oid_t oid, type::TypeId type_id,
                 bool nullable)
      : oid_(oid), packed_type_(0) {
    TERRIER_ASSERT(!(type_id == type::TypeId::VARCHAR ||
   ...
   */
  IndexKeyColumn(catalog::indexkeycol_oid_t oid, type::TypeId type_id,
                 bool nullable, uint16_t max_varlen_size)
      : oid_(oid), packed_type_(0) {
    TERRIER_ASSERT(
        type_id == type::TypeId::VARCHAR || type_id == type::TypeId::VARBINARY,
        "Varlen constructor.");
    SetTypeId(type_id);
    SetNullable(nullable);
  ...
  uint16_t GetMaxVarlenSize() const {
    return static_cast<uint16_t>((packed_type_ & MASK_VARLEN) >> OFFSET_VARLEN);
  }

  /**
   * @return type of this key column
   */
  type::TypeId GetType() const {
    return static_cast<type::TypeId>(packed_type_ & MASK_TYPE);
  }
  ...
  bool IsNullable() const {
    return static_cast<bool>(packed_type_ & MASK_NULLABLE);
  }
 ...
 private:
  ...
  void SetMaxVarlenSize(uint16_t max_varlen_size) {
    TERRIER_ASSERT((packed_type_ & MASK_VARLEN) == 0,
                   "Should only set max varlen size once.");
  ...
  void SetTypeId(type::TypeId type_id) {
    TERRIER_ASSERT((packed_type_ & MASK_TYPE) == 0,
                   "Should only set type once.");
    packed_type_ = packed_type_ | (static_cast<uint32_t>(type_id) & MASK_TYPE);
  }

  void SetNullable(bool nullable) {
    TERRIER_ASSERT((packed_type_ & MASK_NULLABLE) == 0,
                   "Should only set nullability once.");
 ...
 * @warning the columns of the IndexKeySchema are assumed to be in comparison
 * order
 */
