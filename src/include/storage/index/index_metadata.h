...
/**
 * Precomputes index-related metadata that can be used to optimize the
 * operations of the various index key types.
 */
class IndexMetadata {
 public:
  /**
  ...
  explicit IndexMetadata(IndexKeySchema key_schema)
      : key_schema_(std::move(key_schema)),
        ...
        key_oid_to_offset_(ComputeKeyOidToOffset(
            key_schema_, ComputePROffsets(inlined_attr_sizes_))),
        initializer_(
            ProjectedRowInitializer::CreateProjectedRowInitializerForIndexes(
                GetRealAttrSizes(attr_sizes_),
                ComputePROffsets(inlined_attr_sizes_))),
   ...
   */
  const std::vector<IndexKeyColumn> &GetKeySchema() const {
    return key_schema_;
  }

  /**
   * @return unsorted index attribute sizes (key schema order), varlens are
   * marked
   */
   ...
   */
  const std::vector<uint16_t> &GetInlinedAttributeSizes() const {
    return inlined_attr_sizes_;
  }

  /**
   * @return true if the varlens must be inlined, i.e. we cannot rely on
   * VarlenEntry inlining as varlens are too big
   */
   ...
   */
  const std::vector<uint8_t> &GetCompactIntsOffsets() const {
    return compact_ints_offsets_;
  }

  /**
   * @return mapping from key oid to projected row offset
   */
  const std::unordered_map<catalog::indexkeycol_oid_t, uint32_t>
      &GetKeyOidToOffsetMap() const {
   ...
   */
  const ProjectedRowInitializer &GetProjectedRowInitializer() const {
    return initializer_;
  }
   ...
   */
  const ProjectedRowInitializer &GetInlinedPRInitializer() const {
    return inlined_initializer_;
  }
 ...
 private:
  ...
  std::unordered_map<catalog::indexkeycol_oid_t, uint32_t>
      key_oid_to_offset_;                // for execution layer
  ProjectedRowInitializer initializer_;  // user-facing initializer
  ProjectedRowInitializer
      inlined_initializer_;  // for GenericKey, internal only
   ...
   */
  static std::vector<uint8_t> ComputeAttributeSizes(
      const IndexKeySchema &key_schema) {
  ...
  /**
   * Computes the attribute sizes as given by the key schema if everything were
   * inlined. Note varchars are inlined as VarlenEntry if they fit, and as (4
   * bytes of size + varlen content) otherwise. e.g.   if key_schema is
   * {INTEGER, VARCHAR(8), VARCHAR(0), TINYINT, VARCHAR(12)} then attr_sizes
   * returned is {4, 16, 16, 1, 16}
   */
  static std::vector<uint16_t> ComputeInlinedAttributeSizes(
      const IndexKeySchema &key_schema) {
    ...
    for (const auto &key : key_schema) {
      ...
      switch (key_type) {
        ...
        case type::TypeId::VARCHAR: {
          // Add 4 bytes because we'll prepend a size field. If we're too small,
          // we'll just use a VarlenEntry.
          auto varlen_size =
              std::max(static_cast<uint16_t>(key.GetMaxVarlenSize() + 4),
                       static_cast<uint16_t>(sizeof(VarlenEntry)));
          inlined_attr_sizes.emplace_back(varlen_size);
          break;
        }
        default:
          inlined_attr_sizes.emplace_back(
              type::TypeUtil::GetTypeSize(key_type));
          break;
      }
    }
  ...
  /**
   * Computes whether we need to manually inline varlen attributes, i.e. too big
   * for VarlenEntry::CreateInline.
   */
  static bool ComputeMustInlineVarlen(const IndexKeySchema &key_schema) {
    return std::any_of(
        key_schema.begin(), key_schema.end(), [](const auto &key) -> bool {
          switch (key.GetType()) {
            case type::TypeId::VARBINARY:
            case type::TypeId::VARCHAR:
   ...
   */
  static std::vector<uint8_t> ComputeCompactIntsOffsets(
      const std::vector<uint8_t> &attr_sizes) {
    // exclusive scan
   ...
   */
  static std::vector<uint16_t> ComputePROffsets(
      const std::vector<uint16_t> &attr_sizes) {
   ...
   */
  static std::unordered_map<catalog::indexkeycol_oid_t, uint32_t>
  ComputeKeyOidToOffset(const IndexKeySchema &key_schema,
  ...
  /**
   * By default, the uint8_t attr_sizes that we pass around in our system are
   * not the real attribute sizes. The MSB is set to indicate whether a column
   * is VARLEN or otherwise. We mask these off to get the real sizes.
   */
  static std::vector<uint8_t> GetRealAttrSizes(
      std::vector<uint8_t> attr_sizes) {
    std::transform(attr_sizes.begin(), attr_sizes.end(), attr_sizes.begin(),
                   [](uint8_t elem) -> uint8_t {
                     return static_cast<uint8_t>(elem & INT8_MAX);
                   });
    return attr_sizes;
  }
};
