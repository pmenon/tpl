...
class BwTreeIndex final : public Index {
 ...
 private:
  BwTreeIndex(const catalog::index_oid_t oid,
              const ConstraintType constraint_type, IndexMetadata metadata)
 ...
 public:
  ...
  bool Insert(const ProjectedRow &tuple, const TupleSlot location) final {
    TERRIER_ASSERT(GetConstraintType() == ConstraintType::DEFAULT,
                   "This Insert is designed for secondary indexes with no "
  ...
  bool ConditionalInsert(const ProjectedRow &tuple, const TupleSlot location,
    ...
    TERRIER_ASSERT(
        GetConstraintType() == ConstraintType::UNIQUE,
    ...
    const bool ret = bwtree_->ConditionalInsert(index_key, location, predicate,
                                                &predicate_satisfied);
  ...
  void ScanKey(const ProjectedRow &key,
               std::vector<TupleSlot> *value_list) final {
    TERRIER_ASSERT(value_list->empty(),
                   "Result set should begin empty. This can be changed in the "
                   "future if index scan behavior requires it.");
    KeyType index_key;
