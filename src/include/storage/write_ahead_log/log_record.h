 ...
 * on the type of this LogRecord the underlying body can be obtained with the
 * @see GetUnderlyingRecordBodyAs method.
 */
class LogRecord {
 public:
  ...
  /**
   * Get the underlying record body as a certain type, determined from
   * RecordType().
   * @tparam UnderlyingType type of the underlying record body, This must be
   * equal to the return value of the RecordType() call
   * @return pointer to the underlying record body
   */
  template <class UnderlyingType>
  UnderlyingType *GetUnderlyingRecordBodyAs() {
    TERRIER_ASSERT(UnderlyingType::RecordType() == type_,
   ...
   * @param head pointer location to initialize, this is also the returned
   * address (reinterpreted)
   * @param type type of the underlying record
   * @param size size of the entire record, in memory, in bytes
   * @param txn_begin begin timestamp of the transaction that generated this log
   * record
 ...
 private:
  ...
  // This needs to be aligned to 8 bytes to ensure the real size of RedoRecord
  // (plus actual ProjectedRow) is also a multiple of 8.
  uint64_t varlen_contents_[0];
};

// TODO(Tianyu): I don't think this has any effect on correctness, but for
// consistency's sake
static_assert(
    sizeof(LogRecord) % 8 == 0,
    "a projected row inside the log record needs to be aligned to 8 bytes");

/**
 * Record body of a Redo. The header is stored in the LogRecord class that would
 * presumably return this object.
 */
class RedoRecord {
 public:
  ...
  // TODO(Tianyu): Potentially need a setter for Inserts, because we know the
  // TupleSlot after insert
  ...
  /**
   * @return const inlined delta that (was/is to be) applied to the tuple in the
   * table
   */
  ...
  /**
   * @return Size of the entire record of this type, in bytes, in memory, if the
   * underlying Delta is to have the same structure as described by the given
   * initializer.
   */
  ...
  /**
   * Initialize an entire LogRecord (header included) to have an underlying redo
   * record, using the parameters supplied
   * @param head pointer location to initialize, this is also the returned
   * address (reinterpreted)
   * @param txn_begin begin timestamp of the transaction that generated this log
   * record
   ...
   * @return pointer to the initialized log record, always equal in value to the
   * given head
   */
  static LogRecord *Initialize(byte *const head,
    ...
    LogRecord *result = LogRecord::InitializeHeader(
        head, LogRecordType::REDO, Size(initializer), txn_begin);
   ...
   * Hacky back door for BufferedLogReader. Essentially, the current
   * implementation dumps memory content straight out to disk, which we then
   * read directly back into the projected row. After the serialization format
   * is decided, write a real factory method for recovery.
   * @param head
   * @param size
   * @param txn_begin
   * @param table
   ...
   * @return
   */
  static LogRecord *PartialInitialize(byte *const head, const uint32_t size,
                                      const transaction::timestamp_t txn_begin,
                                      DataTable *const table,
                                      TupleSlot tuple_slot) {
    LogRecord *result =
        LogRecord::InitializeHeader(head, LogRecordType::REDO, size, txn_begin);
    auto *body = result->GetUnderlyingRecordBodyAs<RedoRecord>();
    body->table_ = table;
    body->tuple_slot_ = tuple_slot;
    return result;
 ...
 private:
  // TODO(Tianyu): We will eventually need to consult the DataTable to determine
  // how to serialize a given column (varlen? compressed? from an outdated
  // schema?) For now we just assume we can serialize everything out as-is, and
  // the reader still have access to the layout on recovery and can deserialize.
  // This is why we are not just taking an oid.
  DataTable *table_;
...
static_assert(
    sizeof(RedoRecord) % 8 == 0,
    "a projected row inside the redo record needs to be aligned to 8 bytes");

/**
 * Record body of a Delete. The header is stored in the LogRecord class that
 * would presumably return this object.
 */
class DeleteRecord {
 public:
  ...
  /**
   * Initialize an entire LogRecord (header included) to have an underlying
   * delete record, using the parameters supplied.
   *
   * @param head pointer location to initialize, this is also the returned
   * address (reinterpreted)
   * @param txn_begin begin timestamp of the transaction that generated this log
  ...
  static LogRecord *Initialize(byte *const head,
                               const transaction::timestamp_t txn_begin,
                               DataTable *const table, TupleSlot slot) {
    auto *result = LogRecord::InitializeHeader(head, LogRecordType::DELETE,
                                               Size(), txn_begin);
...
/**
 * Record body of a Commit. The header is stored in the LogRecord class that
 * would presumably return this object.
 */
class CommitRecord {
 public:
  ...
  /**
   * Initialize an entire LogRecord (header included) to have an underlying
   * commit record, using the parameters supplied.
   *
   * @param head pointer location to initialize, this is also the returned
   * address (reinterpreted)
   * @param txn_begin begin timestamp of the transaction that generated this log
   * record
   * @param txn_commit the commit timestamp of the transaction that generated
   * this log record
   ...
   * @param is_read_only indicates whether the transaction generating this log
   * record is read-only or not
   * @param txn pointer to the committing transaction
   * @return pointer to the initialized log record, always equal in value to the
   * given head
   */
  // TODO(Tianyu): txn should contain a lot of the information here. Maybe we
  // can simplify the function. Note that however when reading log records back
  // in we will not have a proper transaction.
  static LogRecord *Initialize(byte *const head,
                               const transaction::timestamp_t txn_begin,
                               const transaction::timestamp_t txn_commit,
  ...
  /**
   * @return function pointer of the transaction callback. Not necessarily
   * populated if read back in from disk.
   */
  ...
  /**
   * @return true if and only if the transaction generating this commit record
   * was read-only
   */
 ...
 private:
  ...
  // More specifically, commit timestamp and read_only can be inferred from
  // looking inside the transaction context
