...
/**
 * Stores information about accessing a column using the Arrow format. This
 * includes the type of the column, and pointers to various buffers if the
 * column is variable length or compressed.
 */
struct ArrowColumnInfo {
  ...
  /* end block */
  uint32_t *indices_ =
      nullptr;  // only meaningful for dictionary, pointer to the indices array
};

/**
 * This class encapsulates all the information needed by arrow to interpret a
 * block, such as length, null counts, and the start of varlen columns, etc.
 * (non varlen columns start can be computed from block start and offset for
 * that column, as they are essentially embedded in the block itself)
 *
 * Notice that the information stored in the metadata maybe outdated if the
 * block is hot. (Things like null counts are not well defined independent of a
 * transaction when the block is versioned)
 */
class ArrowBlockMetadata {
 public:
  ...
  static uint32_t Size(uint16_t num_cols) {
    return StorageUtil::PadUpToSize(
               sizeof(uint64_t),
               static_cast<uint32_t>(sizeof(uint32_t)) * (num_cols + 1)) +
           num_cols * static_cast<uint32_t>(sizeof(ArrowColumnInfo));
  }

  void Initialize(uint16_t num_cols) {
    // Need to 0 out this block to make sure all the counts are 0 and all the
  ...
  uint32_t NullCount(col_id_t col_id) const {
    return reinterpret_cast<const uint32_t *>(varlen_content_)[!col_id];
  }

  ArrowColumnInfo &GetColumnInfo(const BlockLayout &layout, col_id_t col_id) {
    byte *null_count_end = storage::StorageUtil::AlignedPtr(
        sizeof(uint64_t),
        varlen_content_ + sizeof(uint32_t) * layout.NumColumns());
    return reinterpret_cast<ArrowColumnInfo *>(null_count_end)[!col_id];
  }

  ArrowColumnInfo &GetColumnInfo(const BlockLayout &layout,
                                 col_id_t col_id) const {
    byte *null_count_end = storage::StorageUtil::AlignedPtr(
        sizeof(uint64_t),
        varlen_content_ + sizeof(uint32_t) * layout.NumColumns());
    return reinterpret_cast<ArrowColumnInfo *>(null_count_end)[!col_id];
 ...
 private:
  ...
  // null_count[num_cols] (32-bit) | padding up to 8 byte-aligned |
