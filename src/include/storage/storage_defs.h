 ...
 * unless interpreted by a TupleAccessStrategy. The header layout is documented
 * in the class as well.
 * @see TupleAccessStrategy
 *
 * @warning If you change the layout please also change the way header sizes are
 * computed in block layout!
 */
struct alignas(common::Constants::BLOCK_SIZE) RawBlock {
  ...
  /**
   * The insert head tells us where the next insertion should take place. Notice
   * that this counter is never decreased as slot recycling does not happen on
   * the fly with insertions. A background compaction process scans through
   * blocks and free up slots.
   */
...
class TupleSlot {
 public:
  ...
  TupleSlot(const RawBlock *const block, const uint32_t offset)
    ...
    TERRIER_ASSERT(
        !((static_cast<uintptr_t>(common::Constants::BLOCK_SIZE) - 1) &
          ((uintptr_t)block)),
        "Address must be aligned to block size (last bits zero).");
    TERRIER_ASSERT(
        offset < common::Constants::BLOCK_SIZE,
  ...
  RawBlock *GetBlock() const {
    ...
    return reinterpret_cast<RawBlock *>(
        bytes_ & ~(static_cast<uintptr_t>(common::Constants::BLOCK_SIZE) - 1));
  }
...
/**
 * Used by execution and storage layers to map between col_oids and offsets
 * within a ProjectedRow
 */
using ProjectionMap = std::unordered_map<catalog::col_oid_t, uint16_t>;

/**
 * Denote whether a record modifies the logical delete column, used when
 * DataTable inspects deltas
...
class VarlenEntry {
 public:
  /**
   * Constructs a new varlen entry. The varlen entry will take ownership of the
   * pointer given if reclaimable is true, which means GC can delete the buffer
   * pointed to when this entry is no longer visible in the storage engine.
   * @param content pointer to the varlen content itself
   * @param size length of the varlen content, in bytes (no C-style
   * nul-terminator)
   * @param reclaim whether the varlen entry's content pointer can be deleted by
   * itself. If the pointer was not allocated by itself (e.g. inlined, or part
   * of a dictionary batch or arrow buffer), it cannot be freed by the GC, which
   * simply calls delete.
   * @return constructed VarlenEntry object
   */
  static VarlenEntry Create(byte *content, uint32_t size, bool reclaim) {
    VarlenEntry result;
    TERRIER_ASSERT(size > InlineThreshold(),
                   "small varlen values should be inlined");
    result.size_ =
        reclaim ? size
                : (INT32_MIN |
                   size);  // the first bit denotes whether we can reclaim it
  ...
  /**
   * Constructs a new varlen entry, with the associated varlen value inlined
   * within the struct itself. This is only possible when the inlined value is
   * smaller than InlineThreshold() as defined. The value is copied and the
   * given pointer can be safely deallocated regardless of the state of the
   * system.
   * @param content pointer to the varlen content
   * @param size length of the varlen content, in bytes (no C-style
   * nul-terminator. Must be smaller than InlineThreshold())
   * @return constructed VarlenEntry object
   */
  static VarlenEntry CreateInline(const byte *content, uint32_t size) {
    TERRIER_ASSERT(size <= InlineThreshold(),
                   "varlen value must be small enough for inlining to happen");
    VarlenEntry result;
    result.size_ = size;
  ...
  /**
   * @return The maximum size of the varlen field, in bytes, that can be inlined
   * within the object. Any objects that are larger need to be stored as a
   * pointer to a separate buffer.
   */
  static constexpr uint32_t InlineThreshold() {
    return sizeof(VarlenEntry) - sizeof(uint32_t);
  }

  /**
   * @return length of the prefix of the varlen stored in the object for
   * execution engine, if the varlen entry is not inlined.
   */
  ...
  bool NeedReclaim() const {
    // force a signed comparison, if our sign bit is set size_ is negative so
 ...
 private:
  int32_t
      size_;  // buffer reclaimable => sign bit is 0 or size <= InlineThreshold
  byte prefix_[sizeof(uint32_t)];  // Explicit padding so that we can use these
                                   // bits for inlined values or prefix
  const byte
      *content_;  // pointer to content of the varlen entry if not inlined
};
// To make sure our explicit padding is not screwing up the layout
static_assert(sizeof(VarlenEntry) == 16,
              "size of the class should be 16 bytes");
...
struct VarlenContentHasher {
  size_t operator()(const VarlenEntry &obj) const {
    return common::HashUtil::HashBytes(obj.Content(), obj.Size());
  }
};
...
struct VarlenContentCompare {
  ...
  bool operator()(const VarlenEntry &lhs, const VarlenEntry &rhs) const {
    ...
    int res = std::memcmp(lhs.Content(), rhs.Content(),
