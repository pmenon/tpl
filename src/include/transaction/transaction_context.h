...
class TransactionContext {
 public:
  /**
   * Constructs a new transaction context. Beware that the buffer pool given
   * must be the same one the log manager uses, if logging is enabled.
   * // TODO(Tianyu): We can terrier assert the above condition, but I need to
   * go figure out friends.
   * @param start the start timestamp of the transaction
   * @param txn_id the id of the transaction, should be larger than all start
   * time and commit time
   * @param buffer_pool the buffer pool to draw this transaction's undo buffer
   * from
   * @param log_manager pointer to log manager in the system, or nullptr, if
   * logging is disabled
   */
  ...
  /**
   * Reserve space on this transaction's undo buffer for a record to log the
   * update given
   ...
   * @return a persistent pointer to the head of a memory chunk large enough to
   * hold the undo record
   */
  storage::UndoRecord *UndoRecordForUpdate(storage::DataTable *const table,
    ...
    return storage::UndoRecord::InitializeUpdate(
        undo_buffer_.NewEntry(size), txn_id_.load(), slot, table, redo);
  }
  ...
  storage::UndoRecord *UndoRecordForInsert(storage::DataTable *const table,
    ...
    return storage::UndoRecord::InitializeInsert(result, txn_id_.load(), slot,
                                                 table);
  }
   ...
   * hold the undo record
   */
  storage::UndoRecord *UndoRecordForDelete(storage::DataTable *const table,
                                           const storage::TupleSlot slot) {
    byte *result = undo_buffer_.NewEntry(sizeof(storage::UndoRecord));
    return storage::UndoRecord::InitializeDelete(result, txn_id_.load(), slot,
                                                 table);
  }

  /**
   * Expose a record that can hold a change, described by the initializer given,
   * that will be logged out to disk. The change can either be copied into this
   * space, or written in the space and then used to change the DataTable.
   * // TODO(Matt): this isn't ideal for Insert since have to call that first
   * and then log it after have a TupleSlot, but it is safe and correct from WAL
   * standpoint
  ...
  storage::RedoRecord *StageWrite(
    ...
    auto *log_record = storage::RedoRecord::Initialize(
        redo_buffer_.NewEntry(size), start_time_, table, slot, initializer);
   ...
   * @param slot the slot that this record changes
   */
  void StageDelete(storage::DataTable *const table,
                   const storage::TupleSlot slot) {
    uint32_t size = storage::DeleteRecord::Size();
    storage::DeleteRecord::Initialize(redo_buffer_.NewEntry(size), start_time_,
                                      table, slot);
  }
 ...
 private:
  ...
  // TODO(Tianyu): Maybe not so much of a good idea to do this. Make explicit
  // queue in GC?
  //
  std::vector<const byte *> loose_ptrs_;
  // log manager will set this to be true when log records are processed (not
  // necessarily flushed, but will not be read again in the future), so it can
