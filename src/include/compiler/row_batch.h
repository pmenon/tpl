 ...
 public:
  ...
  class AttributeAccess {
   public:
    ...
    virtual compiler::Value Access(CodeGen &codegen, Row &row) = 0;
  };
  ...
  class ExpressionAccess : public AttributeAccess {
   public:
    // Constructor
    ExpressionAccess(const parser::AbstractExpression &expression);
   ...
   private:
    ...
    class CacheKey {
     public:
      ...
      CacheKey(const parser::AbstractExpression *expr)
      ...
      struct Hasher {
        size_t operator()(const CacheKey &k) const {
          if (k.ai_ != nullptr) {
            ...
            return std::hash<const parser::AbstractExpression *>{}(k.expr_);
          }
        }
      };
    ...
    std::unordered_map<CacheKey, compiler::Value, CacheKey::Hasher> cache_;
