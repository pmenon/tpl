...
class Memory {
 public:
  ...
  [[nodiscard]] static void *MallocAligned(const std::size_t size, const std::size_t alignment) {
    TPL_ASSERT(util::MathUtil::IsPowerOf2(alignment), "Alignment must be a power of two");
    TPL_ASSERT(util::MathUtil::IsAligned(alignment, sizeof(void *)),
    ...
    ptr = std::aligned_alloc(alignment, util::MathUtil::AlignTo(size, alignment));
#endif
  ...
  static void Prefetch(const void *const addr) {
