...
// Some platforms would have already defined the macro. But its presence is not
// standard and thus not portable. Pretty sure this is always 8 bits. If not,
// consider getting a new machine, and preferably not from another dimension :)
 ...
 * sizes, because we expect it to be reinterpreted from raw memory bytes.
 *
 * Therefore, you should never construct an instance of a RawBitmap. Reinterpret
 * an existing block of memory that you know will be a valid bitmap.
 *
...
class RawBitmap {
 public:
   ...
   * @return the size of the bitmap holding the given number of elements, in
   * bytes.
   */
  static constexpr uint32_t SizeInBytes(uint32_t n) {
   ...
   */
  static void Deallocate(RawBitmap *const map) {
