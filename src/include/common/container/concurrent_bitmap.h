...
class RawConcurrentBitmap {
 public:
  ...
  /**
   * Allocates a new RawConcurrentBitmap of size num_bits. Up to the caller to
   * call Deallocate on its return value
  ...
  static RawConcurrentBitmap *Allocate(const uint32_t num_bits) {
    ...
    TERRIER_ASSERT(reinterpret_cast<uintptr_t>(result) % sizeof(uint64_t) == 0,
                   "Allocate should be 64-bit aligned.");
    std::memset(result, 0, num_bytes);
    return reinterpret_cast<RawConcurrentBitmap *>(result);
  }

  /**
   * Deallocates a RawConcurrentBitmap. Only call on pointers given out by
   * Allocate
   * @param map the map to deallocate
   */
  static void Deallocate(RawConcurrentBitmap *const map) {
  ...
  bool Test(const uint32_t pos) const {
    return static_cast<bool>(bits_[pos / BYTE_SIZE].load() &
                             LSB_ONE_HOT_MASK(pos % BYTE_SIZE));
  }
   ...
   * We search beginning from start_pos. It does not wrap back if it runs out of
   * bits. Note that this result is immediately stale. Furthermore, this
   * function assumes byte 0 is aligned to 64 bits.
   * @param bitmap_num_bits number of bits in the bitmap.
   * @param start_pos start searching from this bit location.
   * @param[out] out_pos the position of the first unset bit will be written
   * here, if it exists.
   * @return true if an unset bit was found, and false otherwise.
   */
  bool FirstUnsetPos(const uint32_t bitmap_num_bits, const uint32_t start_pos,
                     uint32_t *const out_pos) const {
    // invalid starting position.
    if (start_pos >= bitmap_num_bits) {
      return false;
    }

    TERRIER_ASSERT(reinterpret_cast<uintptr_t>(bits_) % sizeof(uint64_t) == 0,
                   "bits_ should be 64-bit aligned.");

    const uint32_t num_bytes = RawBitmap::SizeInBytes(
        bitmap_num_bits);  // maximum number of bytes in the bitmap
    uint32_t byte_pos = start_pos / BYTE_SIZE;  // current byte position
    uint32_t bits_left =
        bitmap_num_bits - start_pos;  // number of bits remaining
    ...
    while (byte_pos < num_bytes && bits_left > 0) {
      ...
      if (!found_unset_bit && IsAlignedAndFits<uint64_t>(bits_left, byte_pos)) {
      ...
      } else if (!found_unset_bit &&
                 IsAlignedAndFits<uint32_t>(bits_left, byte_pos)) {
        found_unset_bit = FindUnsetBit<int32_t>(&byte_pos, &bits_left);
      } else if (!found_unset_bit &&
                 IsAlignedAndFits<uint16_t>(bits_left, byte_pos)) {
        ...
        if (static_cast<std::atomic<int8_t>>(bits) != -1) {
          ...
          // if the bit is valid, we return the bit. however, the bit could be
          // invalid:
          // 1. it could be part of our end padding, 2. it could be before
          // start_pos in which case we must continue to search the next byte,
          // if it exists. we don't bother ensuring result freshness since our
          // returned position is immediately stale.
          for (uint32_t pos = 0; pos < BYTE_SIZE; pos++) {
            uint32_t current_pos = pos + byte_pos * BYTE_SIZE;
            // we are always padded to a byte, but we don't want to use the
            // padding.
        ...
        byte_pos += 1;
        // Also decrease bits_left, making sure that start_pos is taken into
        // account
   ...
   * @param num_bits number of bits to clear. This should be equal to the number
   * of elements of the entire bitmap or unintended elements may be cleared
 ...
 private:
  ...
  /**
   * Looks for an unset bit in a T-sized word from the bitmap, starting at
   * byte_pos. If an unset bit is found, returns true. Otherwise updates
   * byte_pos to be the next place we should search.
  ...
  /**
   * Returns true if a word of size T would both fit and be aligned for the
   * given parameters. This assumes that byte_pos is aligned for 64 bits at 0.
   * @tparam T signed fixed width integer type.
   ...
   */
  template <class T>
  bool IsAlignedAndFits(const uint32_t bits_left,
                        const uint32_t byte_pos) const {
