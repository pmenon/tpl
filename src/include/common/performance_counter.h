 ...
 * The actual counters will be class members defined by the
 * MAKE_PERFORMANCE_COUNTER macro. Accessing and updating counters is minimal
 * cost as they are implemented as atomic int class members.
 */
class PerformanceCounter {
 public:
   ...
   * Note that the JSON object does not automatically update, i.e. result
   * immediately stale.
   ...
   * Undefined behavior occurs if the JSON snapshot and the performance counter
   * differ in structure.
   */
...
/**
 * This macro defines class members by wrapping MemberType in std::atomic and
 * initializing it to 0. MemberType should be an integral type and MemberName
 * should be a valid variable name.
 *
 * We did not find use-cases for non-zero default values and therefore removed
 * that functionality, but extending this macro to support them is
 * straightforward if need arises.
 */
#define PC_HELPER_DEFINE_MEMBERS(MemberType, MemberName) \
  std::atomic<MemberType> MemberName{0};

/**
 * This macro defines a GetMemberName() function which returns the value of
 * MemberName. If performance counters are disabled, it always returns 0.
 */
#define PC_HELPER_DEFINE_GET(MemberType, MemberName) \
  MemberType Get##MemberName() { return MemberName.load(); }

/**
 * This macro defines a SetMemberName(MemberType x) function which sets the
 * value of MemberName to x. If performance counters are disabled, it should do
 * nothing.
 */
...
/**
 * This macro defines an IncrementMemberName(MemberType x) function which
 * increments the value of MemberName by x. If performance counters are
 * disabled, it should do nothing.
 */
#define PC_HELPER_DEFINE_INCREMENT(MemberType, MemberName)                   \
  void Increment##MemberName(MemberType x) {                                 \
    return MemberName.store(static_cast<MemberType>(MemberName.load() + x)); \
  }

/**
 * This macro defines a DecrementMemberName(MemberType x) function which
 * decrements the value of MemberName by x. If performance counters are
 * disabled, it should do nothing.
 */
...
#define PC_FN_JSON_FROM(MemberType, MemberName) \
  MemberName.store(j.at("Counters").at(#MemberName).get<MemberType>());
...
#define PC_FN_JSON_TO(MemberType, MemberName) \
  output["Counters"][#MemberName] = MemberName.load();
...
/*
 * This macro creates a PerformanceCounter in its current namespace as the class
 * ClassName.
 *
 * The actual counters are defined by passing in MemberList, a macro of the form
 *      #define MEMBER_LIST(f) f(type1, name1) f(type2, name2) ... f(typeN,
 * nameN) Note that all of the types should be integral types. An example:
 *      #define NETWORK_MEMBERS(f) f(uint64_t, RequestsReceived) f(uint32_t,
 * ConnectionsOpened)
 *
 * You can use the resulting PerformanceCounter as a class in its own right. For
 * example, #define DEFINE_PERFORMANCE_CLASS(NetworkCounter, NETWORK_MEMBERS)
...
// If you include a header file in multiple translation units, the multiple
// definitions conflict and you get linker errors. We mark it inline so that the
