...
struct Constants {
  ...
  /**
   * Maximum number of columns a table is allowed to have. It should be
   * sufficiently small such that  if all columns are as large as they can be
   * there is still at last one slot for every block.
   */
  // TODO(Tianyu): This number currently is obtained through empirical
  // experiments.
