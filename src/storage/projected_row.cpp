...
ProjectedRow *ProjectedRow::CopyProjectedRowLayout(void *head,
  ...
  auto header_size = reinterpret_cast<uintptr_t>(&other.Bitmap()) -
...
ProjectedRowInitializer::ProjectedRowInitializer(
    const std::vector<AttrType> &attr_sizes, std::vector<col_id_t> col_ids)
    : col_ids_(std::move(col_ids)), offsets_(col_ids_.size()) {
  TERRIER_ASSERT(!col_ids_.empty(), "Cannot initialize an empty ProjectedRow.");
  TERRIER_ASSERT(col_ids_.size() == attr_sizes.size(),
                 "Attribute sizes should correspond to the column indexes");
  TERRIER_ASSERT(
      std::is_sorted(attr_sizes.cbegin(), attr_sizes.cend(), std::greater<>()),
      "Attribute sizes must be sorted descending.");
  TERRIER_ASSERT(
      (std::set<col_id_t>(col_ids_.cbegin(), col_ids_.cend())).size() ==
          col_ids_.size(),
      "There should not be any duplicates in the col_ids.");
  // TODO(Tianyu): We should really assert that it has a subset of columns, but
  // that is a bit more complicated.

  size_ = sizeof(ProjectedRow);  // size and num_col size
  // space needed to store col_ids, must be padded up so that the following
  // offsets are aligned
  size_ = StorageUtil::PadUpToSize(
      sizeof(uint32_t),
      size_ + static_cast<uint32_t>(col_ids_.size() * sizeof(uint16_t)));
  // space needed to store value offsets, we don't need to pad as we're using a
  // regular non-concurrent bitmap
  size_ = size_ + static_cast<uint32_t>(col_ids_.size() * sizeof(uint32_t));
  // Pad up to either the first value's size, or 8 bytes if the value is larger
  // than 8
  auto first_alignment = static_cast<uint8_t>(
      std::min(attr_sizes[0], static_cast<AttrType>(sizeof(uint64_t))));
  // space needed to store the bitmap, padded up to the size of the first value
  // in this projected row
  size_ = StorageUtil::PadUpToSize(
      first_alignment, size_ + common::RawBitmap::SizeInBytes(
                                   static_cast<uint32_t>(col_ids_.size())));
  for (uint32_t i = 0; i < col_ids_.size(); i++) {
    offsets_[i] = size_;
    // Pad up to either the next value's size, or 8 bytes at the end of the
    // ProjectedRow, or 8 byte if the value is larger than 8
    auto next_alignment = static_cast<uint8_t>(
        i == col_ids_.size() - 1
            ? sizeof(uint64_t)
            : std::min(attr_sizes[i + 1],
                       static_cast<AttrType>(sizeof(uint64_t))));
...
ProjectedRowInitializer ProjectedRowInitializer::CreateProjectedRowInitializer(
    const BlockLayout &layout, std::vector<col_id_t> col_ids) {
  TERRIER_ASSERT(col_ids.size() < layout.NumColumns(),
                 "ProjectedRow should have fewer columns than the table (can't "
                 "read version vector)");
  // Sort the projection list for optimal space utilization and delta
  // application performance If the col ids are valid ones laid out by
  // BlockLayout, ascending order of id guarantees descending order in attribute
  // size.
...
template ProjectedRowInitializer
ProjectedRowInitializer::CreateProjectedRowInitializerForIndexes(
