...
BlockLayout::BlockLayout(std::vector<uint8_t> attr_sizes)
  ...
  for (uint8_t size UNUSED_ATTRIBUTE : attr_sizes_)
    TERRIER_ASSERT(size == VARLEN_COLUMN || (size >= 0 && size <= INT8_MAX),
                   "Invalid size of a column");
  TERRIER_ASSERT(
      !attr_sizes_.empty() && static_cast<uint16_t>(attr_sizes_.size()) <=
                                  common::Constants::MAX_COL,
  ...
  // skip the reserved columns because we still want those first and shouldn't
  // mess up 8-byte alignment
  std::sort(attr_sizes_.begin() + NUM_RESERVED_COLUMNS, attr_sizes_.end(),
            std::greater<>());
...
uint32_t BlockLayout::ComputeStaticHeaderSize() const {
  auto unpadded_size = static_cast<uint32_t>(
      sizeof(uint32_t) * 2  // layout_version, insert_head
...
uint32_t BlockLayout::ComputeNumSlots() const {
  uint32_t bytes_available =
      common::Constants::BLOCK_SIZE - static_header_size_;
  // account for paddings up to 64 bits-aligned. There is padding between every
  // bitmap and value field. Each column has a bitmap and a value buffer. The
  // first column can have padding against header. The last column has nothing
  // to pad to.
  bytes_available -= static_cast<uint32_t>(sizeof(uint64_t)) * 2 * NumColumns();
  // Every column needs a bit for bitmap, plus a global presence bit for the
  // whole tuple
