...
std::vector<col_id_t> SqlTable::ColIdsForOids(
  ...
  TERRIER_ASSERT(!col_oids.empty(),
                 "Should be used to access at least one column.");
  std::vector<col_id_t> col_ids;

  // Build the input to the initializer constructor
  for (const catalog::col_oid_t col_oid : col_oids) {
    TERRIER_ASSERT(table_.column_map.count(col_oid) > 0,
...
ProjectionMap SqlTable::ProjectionMapForInitializer(
  ...
  for (uint16_t i = 0; i < initializer.NumColumns(); i++) {
    ...
    // find the key (col_oid) in the table's map corresponding to the value
    // (col_id)
    const auto oid_to_id =
        std::find_if(table_.column_map.cbegin(), table_.column_map.cend(),
                     [&](const auto &oid_to_id) -> bool {
                       return oid_to_id.second == col_id_at_offset;
                     });
...
template ProjectionMap
SqlTable::ProjectionMapForInitializer<ProjectedColumnsInitializer>(
