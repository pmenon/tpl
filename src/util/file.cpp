...
void File::Initialize(const std::filesystem::path &path, uint32_t flags) {
  ...
  if (flags & FLAG_CREATE_ALWAYS) {
    TPL_ASSERT(!open_flags, "CreateAlways is mutually exclusive with other Open/Create flags");
    TPL_ASSERT(flags & FLAG_WRITE, "CreateAlways must include Write flag");
    open_flags = O_CREAT | O_TRUNC;
  }

  if (flags & FLAG_OPEN_TRUNCATED) {
    TPL_ASSERT(!open_flags, "OpenTruncated is mutually exclusive with other Open/Create flags");
    TPL_ASSERT(flags & FLAG_WRITE, "OpenTruncated must include Write flag");
    open_flags = O_TRUNC;
  }

  if (!open_flags && !(flags & FLAG_OPEN) && !(flags & FLAG_OPEN_ALWAYS)) {
    TPL_ASSERT(false, "Must provide one of Open/Create/OpenAlways/CreateAlways/OpenTruncated");
    errno = EOPNOTSUPP;
    error_ = Error::FAILED;
    return;
  }
  ...
  if (flags & FLAG_WRITE && flags & FLAG_READ) {
    ...
    TPL_ASSERT(false, "Flags must include one of Read, Append, or OpenAlways");
  }
...
void File::Create(const std::filesystem::path &path) {
  Open(path, FLAG_CREATE_ALWAYS | FLAG_WRITE);
}
