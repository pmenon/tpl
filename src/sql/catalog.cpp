...
template <typename T>
T *CreateNumberColumnData(ColumnInsertMeta *col_meta, uint32_t num_vals, T min, T max) {
...
std::pair<byte *, uint32_t *> GenerateColumnData(ColumnInsertMeta *col_meta, uint32_t num_rows) {
  ...
  switch (col_meta->sql_type.GetId()) {
    ...
    case SqlTypeId::TinyInt: {
      col_data = reinterpret_cast<byte *>(CreateNumberColumnData<int8_t>(
          col_meta, num_rows, std::get<int64_t>(col_meta->min), std::get<int64_t>(col_meta->max)));
      break;
    }
    ...
    case SqlTypeId::Real: {
      col_data = reinterpret_cast<byte *>(CreateNumberColumnData<float>(
          col_meta, num_rows, std::get<double>(col_meta->min), std::get<double>(col_meta->max)));
      break;
    }
