...
void VectorFilterExecutor::SelectEqVal(const uint32_t col_idx, const GenericValue &val) {
  auto left_vector = vector_projection_->GetColumn(col_idx);
...
void VectorFilterExecutor::SelectEqVal(uint32_t col_idx, const Val &val) {
  auto left_vector = vector_projection_->GetColumn(col_idx);
  auto constant = GenericValue::CreateFromRuntimeValue(left_vector->type_id(), val);
...
void VectorFilterExecutor::SelectEq(const uint32_t left_col_idx, const uint32_t right_col_idx) {
  auto left_vector = vector_projection_->GetColumn(left_col_idx);
  auto right_vector = vector_projection_->GetColumn(right_col_idx);
