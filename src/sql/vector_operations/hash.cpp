...
void TemplatedHashOperation(const Vector &input, Vector *result) {
  ...
  if (input.GetNullMask().Any()) {
    VectorOps::Exec(input, [&](uint64_t i, uint64_t k) {
      result_data[i] = tpl::sql::Hash<InputType>{}(input_data[i], input.GetNullMask()[i]);
    });
  } else {
    VectorOps::Exec(input, [&](uint64_t i, uint64_t k) {
      result_data[i] = tpl::sql::Hash<InputType>{}(input_data[i], false);
    });
  }
}
...
void TemplatedHashCombineOperation(const Vector &input, Vector *result) {
  ...
  if (input.GetNullMask().Any()) {
    VectorOps::Exec(input, [&](uint64_t i, uint64_t k) {
      result_data[i] =
          tpl::sql::HashCombine<InputType>{}(input_data[i], input.GetNullMask()[i], result_data[i]);
    });
  } else {
    VectorOps::Exec(input, [&](uint64_t i, uint64_t k) {
