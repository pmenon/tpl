...
class JoinManagerTest : public TplTest {};
...
TEST_F(JoinManagerTest, TwoWayJoin) {
  ...
  query_state.jht2 = std::make_unique<JoinHashTable>(&mem_pool, sizeof(JoinRow), false);
  query_state.jm = std::make_unique<JoinManager>(&query_state);
  // The first join.
  query_state.jm->InsertJoinStep(*query_state.jht1, {0}, [](auto vp, auto tids, auto ctx) {
    auto *query_state = reinterpret_cast<QueryState *>(ctx);
    query_state->jm->PrepareSingleJoin(vp, tids, 0);
  });
  // The second join.
  query_state.jm->InsertJoinStep(*query_state.jht2, {1}, [](auto vp, auto tids, auto ctx) {
    auto *query_state = reinterpret_cast<QueryState *>(ctx);
  ...
  vec_proj.Reset(kDefaultVectorSize);
