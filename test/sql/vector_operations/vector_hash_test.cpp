  ...
  {                                                                                  \
    ...
    VectorOps::Exec(*input, [&](uint64_t i, uint64_t k) {                            \
      EXPECT_EQ(reinterpret_cast<hash_t *>(hash->data())[i],                         \
                util::Hasher::Hash(reinterpret_cast<CPP_TYPE *>(input->data())[i])); \
    });                                                                              \
  }
...
TEST_F(VectorHashTest, HashWithNullInput) {
  ...
  EXPECT_EQ(util::Hasher::Hash(raw_input[0]), raw_hash[0]);
  EXPECT_EQ(util::Hasher::Hash(raw_input[1]), raw_hash[1]);
