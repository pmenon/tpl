...
TEST(BitfieldTest, MultiElementTest) {
  ...
  using U16_BF = BitField32<uint16_t, 0, sizeof(uint16_t) * kBitsPerByte>;
  using U8_BF = BitField32<uint8_t, U16_BF::kNextBit, sizeof(uint8_t) * kBitsPerByte>;
...
TEST(BitfieldTest, NegativeIntegerTest) {
  // Encode a 16-bit value and an 8-bit value in 32-bit storage
  using U16_BF = BitField32<int16_t, 0, sizeof(uint16_t) * kBitsPerByte>;
